...
class formater:
    ...
    def HeadSetup(self, Bytearray, Type, Unit, TotalUnits):
        ...
        Bytearray = self.addNumber(Bytearray, calcCrC, self.headcrc)
        length = self.headsize + self.headtype + self.headunit + self.headunittotal + self.headcrc
    ...
    def extract_head_raw(self, Bytearray):
        ...
        lista_coisas_head = [self.headcrc, self.headtype, self.headunit, self.headunittotal, self.headsize]
    ...
    def parsemessage(self, Bytearray):
        ...
        payload_eop, headcrc, message_type, unit, TotalUnits, payload_size = self.takeHead(Bytearray)
        temporary_array = b""
        ...
        if (message_type == 4) or (message_type == 6):
                    ...
                    if i < len(payload_eop) - len(self.EOP) + 1:
                        if payload_eop[i:i + len(self.EOP)] == self.EOP:
                            if temporary_array[-3:] == self.escape_byte:
                                ...
                                if (message_type == 4):
                                    calcCrC_check = CalculaCrC(payload_eop[:i])
                                    print("CRC informado: {}".format(headcrc))
                                    print("CRC calculado: {}".format(calcCrC_check))
                                    if headcrc != calcCrC_check:
                                        print("Erro CrC recebido: {}, calculado: {}...".format(headcrc, calcCrC_check))
                                        return (b"", -3, unit, TotalUnits)
            ...
            print("recebido tipo {}".format(message_type))
